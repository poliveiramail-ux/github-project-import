...
export default function SimulationForm_CollapsibleMonths({ onMenuClick }: Props) {
  ...
  const [openMonths, setOpenMonths] = useState<Set<number>>(new Set([1, 2]));
  ...
  const toggleMonth = (month: number) => setOpenMonths(prev => { const n = new Set(prev); n.has(month) ? n.delete(month) : n.add(month); return n; });
    ...
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b sticky top-0 z-20">
        <div className="container mx-auto px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          <Button variant="default" size="sm" className="h-8" onClick={handleSave} disabled={saveStatus === 'saving' || variableValues.size === 0}>
                        ...
                        {uniqueVars.map((uv, idx) => {
                          ...
                          return (
                            <tr key={idx} className="border-b last:border-0 hover:bg-muted/30">
                              <td className="px-4 py-1" style={{ paddingLeft: `${16 + uv.level * 12}px` }}>{uv.name}</td>
                              <td className="px-4 py-1">{uv.id_lang || '-'}</td>
                              <td className="px-4 py-1">{uv.id_lob || '-'}</td>
